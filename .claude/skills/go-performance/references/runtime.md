...
```bash
GOGC=50  ./task-tracker  # latency-sensitive: more frequent, shorter GC pauses
GOGC=200 ./task-tracker  # throughput-oriented: less frequent GC, more memory used
GOGC=off ./task-tracker  # disable GC entirely (testing only!)
```
...
GOMEMLIMIT=450MiB ./task-tracker

# Container with 1GB limit
GOMEMLIMIT=900MiB ./task-tracker
```
...
```bash
GODEBUG=gctrace=1 ./task-tracker 2>&1 | head -20
```
...
In a container with 2 CPU cores on a 64-core host running Go 1.25+ with **cgroup v2**, `GOMAXPROCS` is correctly set to 2 by default. api targets **Go 1.25**, so the runtime reads the container CPU quota itself — no third-party container-detection library is needed (do not add `go.uber.org/automaxprocs`; uber/* is not a dependency here). For **cgroup v1** environments, validate the detected value at startup; if the runtime mis-detects, fall back to setting `GOMAXPROCS` explicitly (below) rather than pulling in a library.

**Manual override** (if needed):

```bash
GOMAXPROCS=2 ./task-tracker
GODEBUG=updatemaxprocs=0 ./task-tracker  # disable dynamic updates (Go 1.25+)
```
...
2. Place as `default.pgo` in the main package directory:
   ...
   ```bash
   cp cpu.pprof ./cmd/task-tracker/default.pgo
   ```

3. Build — `go build` auto-detects `default.pgo`:

   ```bash
   go build ./cmd/task-tracker
   ```
