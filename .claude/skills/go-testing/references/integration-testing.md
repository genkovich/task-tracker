...
> **beer-lms recipe (use this):** integration tests carry `//go:build integration` and start real Postgres via **`dbtest.StartPostgres(ctx, t)`** (`internal/platform/database/dbtest/container.go`) — testcontainers image **`postgres:18-alpine`**, `t.Skip` when docker is absent, auto-terminate via `t.Cleanup`. Apply schema with **`database.RunMigrations(migrations.FS, c.DSN)`**; use `c.DB` (`*database.DB`). Redis-dependent tests use **miniredis** (`mr := miniredis.RunT(t)`), not a container. Run with `make test-integration` (`go test -tags integration -p 4 ./...`).
>
> ```go
...
>     }
>     repo := infra.NewPostgresCommentRepository(c.DB)
>     // ...
> }
> ```
>
> The docker-compose + `sql.Open` + `suite.Suite` patterns below are the **upstream spelling** (database/sql) — kept for reference. beer-lms does not hand-roll docker-compose fixtures or `sql.Open`; it uses the `dbtest` helper above.
