...
> **Not the beer-lms house style.** This repo uses **hand-written fakes** against consumer-side ports (see `fakeCourseRepo` in `internal/modules/courses/app/service_test.go` and the Mocking reference). Reach for `testify/mock` only when you specifically need argument matchers (`mock.MatchedBy`), call-count verification (`.Times(n)`), or strict call-order checks that a fake would make verbose. For ordinary isolation, write a fake.
