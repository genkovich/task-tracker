...
> **beer-lms note:** the table below is the full upstream catalog by domain. The shipped [`.golangci.yml`](../assets/.golangci.yml) enables a **conservative subset** — the correctness core plus the linters the repo's rules promise — and leaves the high-noise/opinionated ones (gosec, funlen, gocyclo, dupl, wsl_v5, godot, mnd, the `database/sql`-only sqlclosecheck/rowserrcheck, …) OFF for incremental adoption. `copyloopvar` is unnecessary on Go 1.25 (the loop-var capture bug was fixed in 1.22). See the SKILL's "In beer-lms" table for the exact enabled set.

These domains group the available linters:
...
Only the conservative subset is enabled in the beer-lms [`.golangci.yml`](../assets/.golangci.yml); the rest are documented here so you know what each does before enabling it.
