...
> **beer-lms note:** the pool setup below is `database/sql` + sqlx and is **not used here**. beer-lms uses **pgxpool**, configured in `database.New` via the pool config / DSN parameters (`pool_max_conns`, `pool_min_conns`, `pool_max_conn_lifetime`, `pool_max_conn_idle_time`). The *sizing reasoning* in the table below still applies; only the API differs. The sqlx batch-INSERT and metrics-collector snippets further down are reference-only.
...
```go
// database/sql + sqlx — NOT used in beer-lms (reference only). pgxpool equivalent:
//   dsn := "postgres://…?pool_max_conns=25&pool_max_conn_lifetime=5m&pool_max_conn_idle_time=1m"
