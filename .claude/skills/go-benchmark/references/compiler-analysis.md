...
GOSSAFUNC=task-tracker/pkg/parser.Parse go build ./...
...
go build -gcflags="all=-S" ./... 2>&1 | grep -A 50 'task-tracker/pkg/parser.Parse'

# Disassemble a compiled binary (alternative to -gcflags="-S")
go build -o task-tracker ./cmd/server
go tool objdump -s Parse task-tracker

# Disassemble with source interleaving
go tool objdump -S -s Parse task-tracker

# Disassemble a specific symbol
go tool objdump -s 'task-tracker/pkg/parser.Parse' task-tracker

# Disassemble a specific text range (by address)
go tool objdump -start 0x4a3b00 -end 0x4a3c00 task-tracker

# List all symbols in a binary
go tool nm task-tracker | grep Parse
