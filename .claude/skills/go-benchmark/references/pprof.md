...
Showing nodes accounting for 4.2s, 84% of 5s total
     ...
     0.40s  8.00% 76.00%      1.90s 38.00%  task-tracker/pkg/parser.Parse
...
(pprof) top -cum
      flat  flat%   sum%        cum   cum%
     0.40s  8.00%  8.00%      3.80s 76.00%  task-tracker/pkg/handler.HandleRequest
     0.10s  2.00% 10.00%      2.80s 56.00%  task-tracker/pkg/handler.serializeResponse
...
Total: 5s
ROUTINE ======================== task-tracker/pkg/handler.serializeResponse
...
----------------------------------------------+-------------
                                               |      flat  flat%   sum%        cum   cum%
 task-tracker/pkg/handler.serializeResponse 2.60s     |
 task-tracker/pkg/api.buildResponse         0.20s     |     1.50s 30.00% 30.00%      2.80s 56.00%  encoding/json.Marshal
----------------------------------------------+-------------
                                               |
...
(pprof) tree
     0.40s  8.00%  8.00%      3.80s 76.00%  task-tracker/pkg/handler.HandleRequest
              0.10s  task-tracker/pkg/handler.serializeResponse
                     1.50s  encoding/json.Marshal
                            0.80s  runtime.mallocgc
              0.20s  task-tracker/pkg/handler.validateInput
              0.10s  task-tracker/pkg/handler.fetchData
```
...
-----------+-------------------------------------------------------
     bytes:  1.5MB
     1.50s   encoding/json.Marshal
             task-tracker/pkg/handler.serializeResponse
             task-tracker/pkg/handler.HandleRequest
...
(pprof) disasm Parse
Total: 5s
ROUTINE ======================== task-tracker/pkg/parser.Parse
...
```
(pprof) focus=task-tracker            # only show call paths that pass through "task-tracker"
...
go tool pprof -focus=task-tracker/pkg/handler -top cpu.prof
