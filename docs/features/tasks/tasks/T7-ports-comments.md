...
- [ ] неіснуюча задача → 404 `task.not_found` (не 500 від FK і не порожній список)
- [ ] неіснуючий коментар → 404 `comment.not_found`
- [ ] коментар не видаляється через шлях чужої задачі: ключ — пара
      (`taskId`, `commentId`), інакше `taskId` у маршруті нічого не значить
