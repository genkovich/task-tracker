 ...
 * for both touch and mouse alike. The card captures the pointer, follows it
 * with a transform once past the threshold, and reports start/move/end up to
 * `useBoardDnd` (wired at the page level). */
export function TaskCard({
  ...
  function handlePointerMove(e: React.PointerEvent<HTMLDivElement>) {
    ...
    if (!draggingRef.current) {
      ...
      const el = cardRef.current;
      if (el) {
        // pointer-events:none keeps the moving card out of elementFromPoint,
        // so useBoardDnd resolves the column underneath it; the captured
        // pointer still delivers events here (capture bypasses hit testing).
        el.style.zIndex = "50";
