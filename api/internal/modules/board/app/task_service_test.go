...
func (r *fakeRepo) MoveTask(_ context.Context, taskID, columnID uuid.UUID) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func TestEditTask_UpdatesTitleAndAssignee(t *testing.T) {
	...
	leftmostID := uuid.Must(uuid.NewV7())
	repo := newFakeRepo(boardID, leftmostID)
	existing := domain.Task{ID: uuid.Must(uuid.NewV7()), ColumnID: leftmostID, Title: "Original title"}
...
func TestMoveTask_ValidColumn_UpdatesColumnID(t *testing.T) {
	...
	err := svc.MoveTask(context.Background(), existing.ID, targetID)
...
func TestMoveTask_InvalidColumn_RejectedNoWriteNoBroadcast(t *testing.T) {
	...
	err := svc.MoveTask(context.Background(), existing.ID, invalidColumnID)
